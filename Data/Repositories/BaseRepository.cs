...
public class BaseRepository<TEntity, TModel> : IBaseRepository<TEntity, TModel> where TEntity : class
{
    ...
    {
        if (entity == null)
            return new RepositoryResult<bool> { Succeeded = false, StatusCoode = 400, Error = "Entity cannot be null" };

        try
        {
            _table.Add(entity);
            await _context.SaveChangesAsync();
            return new RepositoryResult<bool> { Succeeded = true, StatusCoode = 201 };
        }
        catch (Exception ex)
        {
            Debug.WriteLine(ex.Message);
            return new RepositoryResult<bool> { Succeeded = false, StatusCoode = 500, Error = ex.Message };
        }
    }
    ...
    public virtual async Task<RepositoryResult<IEnumerable<TModel>>> GetAllAsync(bool orderByDescending = false, Expression<Func<TEntity, object>>? sortBy = null, Expression<Func<TEntity, bool>>? where = null, params Expression<Func<TEntity, object>>[] includes)
    {
        ...
        return new RepositoryResult<IEnumerable<TModel>> { Succeeded = true, StatusCoode = 200, Result = result };
    }
    ...
    public virtual async Task<RepositoryResult<IEnumerable<TSelect>>> GetAllAsync<TSelect>(Expression<Func<TEntity, TSelect>> selector, bool orderByDescending = false, Expression<Func<TEntity, object>>? sortBy = null, Expression<Func<TEntity, bool>>? where = null, params Expression<Func<TEntity, object>>[] includes)
    {
        ...
        return new RepositoryResult<IEnumerable<TSelect>> { Succeeded = true, StatusCoode = 200, Result = result };
    }
    public virtual async Task<RepositoryResult<TModel>> GetAsync(Expression<Func<TEntity, bool>> where, params Expression<Func<TEntity, object>>[] includes)
    {
        ...
        if (entity == null)
            return new RepositoryResult<TModel> { Succeeded = false, StatusCoode = 404, Error = "Entity not found" };

        var result = entity.MapTo<TModel>();
        return new RepositoryResult<TModel> { Succeeded = true, StatusCoode = 200, Result = result };
    }
    public virtual async Task<RepositoryResult<bool>> ExistAsync(Expression<Func<TEntity, bool>> findBy)
    {
        ...
        return !exists
            ? new RepositoryResult<bool> { Succeeded = false, StatusCoode = 404, Error = "Entity not found" }
            : new RepositoryResult<bool> { Succeeded = true, StatusCoode = 200 };
    }
    ...
    public virtual async Task<RepositoryResult<bool>> UpdateAsync(TEntity entity)
    {
        ...
        try
        {
            ...
            await _context.SaveChangesAsync();
            return new RepositoryResult<bool> { Succeeded = true, StatusCoode = 200 };
        }
        catch (Exception ex)
        {
            Debug.WriteLine(ex.Message);
